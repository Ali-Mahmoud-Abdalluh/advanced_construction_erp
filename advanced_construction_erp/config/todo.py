...
from advanced_construction_erp.advanced_construction.doctype.advanced_construction_erp_notification.advanced_construction_erp_notification import notify_user


def after_insert(doc, method):
    if (
        doc.reference_type in ["Advanced Construction ERP Lead", "Advanced Construction ERP Deal"]
        and doc.reference_name
        and doc.allocated_to
    ):
        fieldname = "lead_owner" if doc.reference_type == "Advanced Construction ERP Lead" else "deal_owner"
    ...
    if (
        doc.reference_type in ["Advanced Construction ERP Lead", "Advanced Construction ERP Deal", "Advanced Construction ERP Task"]
        and doc.reference_name
        and doc.allocated_to
    ):
...
def on_update(doc, method):
    if (
        ...
        and doc.reference_type in ["Advanced Construction ERP Lead", "Advanced Construction ERP Deal", "Advanced Construction ERP Task"]
        and doc.reference_name
        and doc.allocated_to
    ):
...
def get_notification_text(owner, doc, reference_doc, is_cancelled=False):
    ...
    if doctype.startswith("Advanced Construction ERP "):
...
def get_redirect_to_doc(doc):
    if doc.reference_type == "Advanced Construction ERP Task":
