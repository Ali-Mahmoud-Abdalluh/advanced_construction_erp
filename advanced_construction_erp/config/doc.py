...
from advanced_construction_erp.config.views import get_views
from advanced_construction_erp.advanced_construction.doctype.advanced_construction_erp_form_script.advanced_construction_erp_form_script import get_form_script
...
def get_quick_filters(doctype: str, cached: bool = True):
	...
	if global_settings := frappe.db.exists("Advanced Construction ERP Global Settings", {"dt": doctype, "type": "Quick Filters"}):
		_quick_filters = frappe.db.get_value("Advanced Construction ERP Global Settings", global_settings, "json")
	...
	if doctype == "Advanced Construction ERP Lead":
...
def create_update_global_settings(doctype, quick_filters):
	if global_settings := frappe.db.exists("Advanced Construction ERP Global Settings", {"dt": doctype, "type": "Quick Filters"}):
		frappe.db.set_value("Advanced Construction ERP Global Settings", global_settings, "json", json.dumps(quick_filters))
	else:
		# create CRM Global Settings doc
		doc = frappe.new_doc("Advanced Construction ERP Global Settings")
		doc.dt = doctype
...
def get_data(
	...
	if view_type != "kanban":
		...
		if not custom_view and frappe.db.exists("Advanced Construction ERP View Settings", default_view_filters):
			list_view_settings = frappe.get_doc("Advanced Construction ERP View Settings", default_view_filters)
	...
	if not is_default and custom_view_name:
		is_default = frappe.db.get_value("Advanced Construction ERP View Settings", custom_view_name, "load_default_columns")
...
def getCounts(d, doctype):
	...
	d["_task_count"] = frappe.db.count(
		"Advanced Construction ERP Task", filters={"reference_doctype": doctype, "reference_docname": d.get("name")}
	)
	d["_note_count"] = frappe.db.count(
		"Advanced Construction ERP Note", filters={"reference_doctype": doctype, "reference_docname": d.get("name")}
	)
	return d