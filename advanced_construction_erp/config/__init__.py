...
def check_app_permission():
	...
	if any(
		role in ["System Manager", "Construction User", "Construction Manager", "Construction Master Manager"] for role in roles
	):
		return True
...
def accept_invitation(key: str | None = None):
	...
	result = frappe.db.get_all("Advanced Construction ERP Invitation", filters={"key": key}, pluck="name")
	if not result:
		frappe.throw("Invalid or expired key")

	invitation = frappe.get_doc("Advanced Construction ERP Invitation", result[0])
	invitation.accept()
	invitation.reload()
	...
	if invitation.status == "Accepted":
		...
		frappe.local.response["location"] = "/advanced_construction_erp"


@frappe.whitelist()
def invite_by_email(emails: str, role: str):
	frappe.only_for("Construction Manager")

	if role not in ["Construction Manager", "Construction User"]:
	...
	existing_invites = frappe.db.get_all(
		"Advanced Construction ERP Invitation",
		filters={"email": ["in", email_list], "role": ["in", ["Construction Manager", "Construction User"]]},
		pluck="email",
	)
	...
	for email in to_invite:
		frappe.get_doc(doctype="Advanced Construction ERP Invitation", email=email, role=role).insert(ignore_permissions=True)
