...
from advanced_construction_erp.api.doc import get_assigned_users
...
def get_lead_or_deal_from_number(number):
	...
	doctype = "Advanced Construction ERP Deal"

	doc = find_record(doctype, number) or None
	if not doc:
		doctype = "Advanced Construction ERP Lead"
...
def get_whatsapp_messages(reference_doctype, reference_name):
	...
	if reference_doctype == "Advanced Construction ERP Deal":
		lead = frappe.db.get_value(reference_doctype, reference_name, "lead")
		if lead:
			messages = frappe.get_all(
				"WhatsApp Message",
				filters={
					"reference_doctype": "Advanced Construction ERP Lead",
					"reference_name": lead,
				},
				fields=[
					"name",
...
def get_from_name(message):
	...
	from_name = ""
	if message["reference_doctype"] == "Advanced Construction ERP Deal":
		if doc.get("contacts"):
