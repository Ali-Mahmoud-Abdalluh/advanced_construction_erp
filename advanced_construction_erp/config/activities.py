...
from advanced_construction_erp.advanced_construction.doctype.advanced_construction_erp_call_log.advanced_construction_erp_call_log import parse_call_log


@frappe.whitelist()
def get_activities(name):
	if frappe.db.exists("Advanced Construction ERP Deal", name):
		return get_deal_activities(name)
	elif frappe.db.exists("Advanced Construction ERP Lead", name):
...
def get_deal_activities(name):
	get_docinfo("", "Advanced Construction ERP Deal", name)
	docinfo = frappe.response["docinfo"]
	deal_meta = frappe.get_meta("Advanced Construction ERP Deal")
	deal_fields = {
	...
	doc = frappe.db.get_values("Advanced Construction ERP Deal", name, ["creation", "owner", "lead"])[0]
	lead = doc[2]
	...
	attachments = attachments + get_attachments("Advanced Construction ERP Deal", name)
...
def get_lead_activities(name):
	get_docinfo("", "Advanced Construction ERP Lead", name)
	docinfo = frappe.response["docinfo"]
	lead_meta = frappe.get_meta("Advanced Construction ERP Lead")
	lead_fields = {
	...
	doc = frappe.db.get_values("Advanced Construction ERP Lead", name, ["creation", "owner"])[0]
	activities = [
		{
			"activity_type": "creation",
			"creation": doc[0],
	...
	attachments = get_attachments("Advanced Construction ERP Lead", name)
...
def get_linked_calls(name):
	calls = frappe.db.get_all(
		"Advanced Construction ERP Call Log",
	...
	linked_calls = frappe.db.get_all(
		"Dynamic Link", filters={"link_name": name, "parenttype": "Advanced Construction ERP Call Log"}, pluck="parent"
	)
	...
	if linked_calls:
		CallLog = frappe.qb.DocType("Advanced Construction ERP Call Log")
		...
		for call in _calls:
			if call.get("link_doctype") == "Advanced Construction ERP Note":
				notes.append(call.link_name)
			elif call.get("link_doctype") == "Advanced Construction ERP Task":
				tasks.append(call.link_name)

		_calls = [call for call in _calls if call.get("link_doctype") not in ["Advanced Construction ERP Note", "Advanced Construction ERP Task"]]
		if _calls:
			calls = calls + _calls

	if notes:
		notes = frappe.db.get_all(
			"Advanced Construction ERP Note",
	...
	if tasks:
		tasks = frappe.db.get_all(
			"Advanced Construction ERP Task",
